...
import os import subprocess import json import time import asyncio import logging import shutil from dotenv import load_dotenv from pyrogram import Client, filters from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup import aria2p import validators

Load environment variables

load_dotenv() api_id = int(os.getenv("API_ID")) api_hash = os.getenv("API_HASH") chat_id = int(os.getenv("CHAT_ID"))
...
prefs_file = "prefs.json" user_prefs = json.load(open(prefs_file)) if os.path.exists(prefs_file) else {} SPLIT_DURATIONS = {"10min": 600, "20min": 1200, "30min": 1800}
...
@app.on_message(filters.document & filters.private) async def handle_torrent(client, message): file = message.document if not file.file_name.endswith(".torrent"): return await message.reply("Please send a valid .torrent file.") path = f"downloads/{file.file_name}" await message.download(file_name=path) user_prefs[message.from_user.id] = {"torrent": path, "type": "torrent"} save_preferences() await message.reply("Select options:", reply_markup=preferences_keyboard())

@app.on_message(filters.text & filters.private) async def handle_links(client, message): text = message.text.strip() user_id = message.from_user.id if text.startswith("magnet:?xt="): user_prefs[user_id] = {"magnet": text, "type": "magnet"} elif validators.url(text): user_prefs[user_id] = {"url": text, "type": "url"} else: return await message.reply("Send a valid magnet or direct link.") save_preferences() await message.reply("Select options:", reply_markup=preferences_keyboard())

@app.on_callback_query() async def on_button(client, query): user_id = query.from_user.id data = query.data msg = query.message
...
def cleanup_slow_downloads(threshold_kbps=5, timeout=60): for download in aria2.get_downloads(): speed = int(download.download_speed or 0) / 1024 if download.is_active and speed < threshold_kbps: if not hasattr(download, "slow_start"): download.slow_start = time.time() elif time.time() - download.slow_start > timeout: logger.info(f"Removing slow: {download.name}") aria2.remove([download], force=True, files=True) else: download.slow_start = time.time()

async def start_download(user_id, message): settings = user_prefs[user_id] download = None try: if settings["type"] == "torrent": download = aria2.add_torrent(settings["torrent"], download_dir="downloads") elif settings["type"] == "magnet": download = aria2.add_magnet(settings["magnet"], download_dir="downloads") elif settings["type"] == "url": download = aria2.add_uris([settings["url"]], download_dir="downloads")
...
async def process_video(message, path): settings = user_prefs[message.from_user.id] max_size = 2 * 1024 * 1024 * 1024 filename = os.path.basename(path)
